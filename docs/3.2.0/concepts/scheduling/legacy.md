...
https://docs.serverpod.dev/3.2.0/concepts/scheduling/legacy

:::warning
This approach is error prone since it involves manually registering and scheduling future calls using string identifiers. The recommended way to interact with the future calls feature is through the [type-safe API](https://docs.serverpod.dev/3.2.0/concepts/scheduling/setup.md).
:::

Creating a future call is simple, extend the `FutureCall` class and override the `invoke` method. The method takes two params the first being the [`Session`](https://docs.serverpod.dev/3.2.0/concepts/sessions.md) object and the second being an optional SerializableModel ([See models](https://docs.serverpod.dev/3.2.0/concepts/models.md)).
