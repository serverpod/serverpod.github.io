...
https://docs.serverpod.dev/2.7.0/concepts/testing/best-practises
...
Unless configured otherwise, by default `withServerpod` does all database operations inside a transaction that is rolled back after each `test` (see [the configuration options](https://docs.serverpod.dev/2.7.0/concepts/testing/the-basics.md#rollback-database-configuration) for more info on this behavior).
