...
https://docs.serverpod.dev/next/how-it-works
...
- **Project type:** A server with a Flutter app (the default), a server only, or a reusable [module](https://docs.serverpod.dev/next/concepts/server-fundamentals/modules.md) shared across servers.
...
In the `_server` package you add your endpoints and data models. Serverpod's code generator generates code on the server and in the client package. You get a type-safe Dart API for your app, along with the serialization and database code on the server. You never write serialization, HTTP calls, or API contracts. For what each package holds in detail, see [Your Serverpod project](https://docs.serverpod.dev/next/concepts/server-fundamentals/your-serverpod-project.md).
...
In Serverpod, endpoints are the entry points your app calls to run code on the server. You define one as a class that extends `Endpoint`, with async methods that each take a [`Session`](https://docs.serverpod.dev/next/concepts/endpoints-and-apis/sessions.md) as their first argument and return a typed `Future` (or a `Stream`, for live updates):
...
The client handles the request, the response, and the JSON in between. Most calls follow this request-and-response shape. For live updates, Serverpod also has [streaming methods](https://docs.serverpod.dev/next/concepts/endpoints-and-apis/streaming.md) that keep a connection open so the server and app can push data to each other.
...
When you generate code, this becomes a `Company` Dart class shared by the server and your app, so the same type flows from your endpoints into your Flutter widgets. Fields support the same types as endpoint methods, plus enums and nested models, and can be nullable. See [Working with models](https://docs.serverpod.dev/next/concepts/data-and-the-database/models.md) for the details.
...
See [CRUD](https://docs.serverpod.dev/next/concepts/data-and-the-database/database/crud.md) for the generated query methods, [relations](https://docs.serverpod.dev/next/concepts/data-and-the-database/database/relations.md) for linking tables, and [transactions](https://docs.serverpod.dev/next/concepts/data-and-the-database/database/transactions.md) for atomic operations.
...
If you want to quickly try out Serverpod and build something with an AI agent, check out our [Quickstart](https://docs.serverpod.dev/next/quickstart.md) guide.

## Next steps

Ready to build something for real? Follow [Build your first app](https://docs.serverpod.dev/next/get-started/creating-endpoints.md) to create your first endpoint and call it from Flutter. When you are ready to ship, [Deploy to Serverpod Cloud](https://docs.serverpod.dev/next/deployments/deploy-to-serverpod-cloud.md) takes your app to production in just a few minutes.
