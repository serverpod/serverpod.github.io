...
The module keeps working on 4.0, so this can be done independently of moving to the new authentication framework. To make that move, see [Migrate from legacy auth](https://docs.serverpod.dev/next/upgrading/migrate-from-legacy-auth.md) after completing this upgrade.
...
New projects use `ghcr.io/serverpod/postgres:16`, which bundles pgvector and PostGIS, instead of `pgvector/pgvector:pg16`. Existing Docker setups keep working; switch the image when you need PostGIS. See [Upgrade to PostGIS](https://docs.serverpod.dev/next/upgrading/upgrade-to-postgis.md).
...
The `dataPath` setting belongs in `development.yaml` and `testing.yaml` only. For production, use a managed Postgres: [Serverpod Cloud](https://docs.serverpod.dev/cloud.md) provisions one for you, or you can connect to a managed service like Cloud SQL or RDS. Do not add `dataPath` to `production.yaml` or `staging.yaml`.
...
- **Opt-in cookie auth for the web.** Enabling the new `authCookie` configuration requires listing every browser origin in `allowedOrigins`; browsers on unlisted origins lose cross-origin access, including to public endpoints. See [web authentication](https://docs.serverpod.dev/next/concepts/authentication/web-authentication.md).
...
- **httpOnly cookie authentication for the web**, keeping browser sign-in tokens out of JavaScript-readable storage. See [web authentication](https://docs.serverpod.dev/next/concepts/authentication/web-authentication.md).
...
Run `serverpod create .` again from the project's root folder. Some editors, like Cursor, require enabling the **Serverpod** and **Dart** MCP servers in their settings (*Cursor Settings* > *Tools & MCPs*).
...
- [Build your first app](https://docs.serverpod.dev/next/get-started/creating-endpoints.md): the hands-on tour of the 4.0 workflow if you want to see `serverpod start` from scratch.
