...
Compile errors after you regenerate are expected. Most match a section on [Breaking changes in 4.0](https://docs.serverpod.dev/next/upgrading/breaking-changes-in-four.md). Plan for more time if breaking changes affect your code, or if you have a Dockerfile or CI workflows to update.
...
- **If you use the legacy `serverpod_auth` module, expect code changes.** It keeps working on 4.0, but the client setup and Sign in with Apple need updating. See [the legacy auth changes](https://docs.serverpod.dev/next/upgrading/breaking-changes-in-four.md#if-you-use-the-legacy-auth-module).
- **If you use the new auth module, check your Flutter app's `flutter_secure_storage` version.** The module needs 10.0.0 or newer, which most projects already have. If yours is still on 9.x, read [how to upgrade without signing out your Android users](https://docs.serverpod.dev/next/upgrading/breaking-changes-in-four.md#if-you-use-the-new-auth-module-on-android) first.
...
**Check your model files first.** Rename each model file that ends in just `.yaml` or `.yml`, so `company.yaml` becomes `company.spy.yaml`. Otherwise, `serverpod generate` [stops with an error](https://docs.serverpod.dev/next/upgrading/breaking-changes-in-four.md#model-files-use-the-spyyaml-extension).
...
1. Find the matching section on [Breaking changes in 4.0](https://docs.serverpod.dev/next/upgrading/breaking-changes-in-four.md).
2. Update the file.
3. Run `serverpod generate` again.

Repeat until the command finishes without errors.

Some changes don't cause an error, such as how future calls and messages behave. Scan the table on [Breaking changes in 4.0](https://docs.serverpod.dev/next/upgrading/breaking-changes-in-four.md) for anything else that affects your project.

## Create the 4.0 migration

Version 4.0 adds a few internal Serverpod tables. It also updates some indexes to speed up logs in [Serverpod Insights](https://docs.serverpod.dev/next/tools/insights.md), the desktop log viewer.
...
If you run the server yourself instead, start it once from the server package with `dart run bin/main.dart --apply-migrations`. See [Run the server directly](https://docs.serverpod.dev/next/concepts/server-fundamentals/running-your-server.md#run-the-server-directly).
...
See [Running your server](https://docs.serverpod.dev/next/concepts/server-fundamentals/running-your-server.md) for the full workflow. Before you run `serverpod start`, choose how to handle the database.
...
- [Breaking changes in 4.0](https://docs.serverpod.dev/next/upgrading/breaking-changes-in-four.md): every API and behavior change between 3.4 and 4.0, for looking up an error.
- [Running your server](https://docs.serverpod.dev/next/concepts/server-fundamentals/running-your-server.md): what the `serverpod start` command does, and the other ways to run your server.
- [Embedded PostgreSQL](https://docs.serverpod.dev/next/concepts/data-and-the-database/database/embedded-postgres.md): how the embedded Postgres runs alongside your server, and how to reset it or connect a tool to it.
