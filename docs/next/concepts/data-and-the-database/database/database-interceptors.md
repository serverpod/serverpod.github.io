...
https://docs.serverpod.dev/next/concepts/data-and-the-database/database/database-interceptors
...
Pass transaction objects, [runtime parameters](https://docs.serverpod.dev/next/concepts/data-and-the-database/database/runtime-parameters.md), ordering, pagination, [row locks](https://docs.serverpod.dev/next/concepts/data-and-the-database/database/row-locking.md), and return options through without changing their meaning. A layer that enforces a policy also needs to cover [raw access](https://docs.serverpod.dev/next/concepts/data-and-the-database/database/raw-access.md) and transactions, otherwise a caller can reach the database around it. Do not hold on to the session or its inner database after the session closes.
