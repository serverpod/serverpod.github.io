...
https://docs.serverpod.dev/next/concepts/operations/health-checks
...
These three paths are reserved. A [web server route](https://docs.serverpod.dev/next/concepts/web-server/routing.md) registered on them would never be reached.
...
- `redis:connection` checks Redis, when it is [enabled](https://docs.serverpod.dev/next/concepts/operations/redis.md).
