...
The `host`, `port`, `user`, `enabled`, and `requireSsl` keys can each be set with environment variables instead of YAML. See the [configuration reference](https://docs.serverpod.dev/next/concepts/lookups/configuration-reference.md) for the full list. You can enable Redis with environment variables alone, with no `redis` block in YAML, if host, port, and password are set.
...
Both variables set the same secret. If both are set, `SERVERPOD_PASSWORD_redis` wins. Environment variables override the passwords file. See [Manage secrets](https://docs.serverpod.dev/next/concepts/server-fundamentals/configuration.md#manage-secrets) for how secrets are loaded.
...
New projects use an [embedded PostgreSQL](https://docs.serverpod.dev/next/concepts/data-and-the-database/database/embedded-postgres.md) (`database.dataPath`), so `serverpod start` does not bring Compose up on its own. Start Redis yourself from the server package:
...
See [Running your server](https://docs.serverpod.dev/next/concepts/server-fundamentals/running-your-server.md) for when Compose is started automatically.
...
On [Serverpod Cloud](https://docs.serverpod.dev/cloud.md), set the same values with `serverpod cloud password set` and `serverpod cloud variable set`. See [Use Redis for PubSub and caching](https://docs.serverpod.dev/cloud/guides/redis.md).
...
For Serverpod Cloud with a third-party Redis such as Upstash, follow [Use Redis for PubSub and caching](https://docs.serverpod.dev/cloud/guides/redis.md).
...
The in-memory fallback for `session.caches.global` is a separate cache, not `local` or `localPrio`. See [The global cache and Redis](https://docs.serverpod.dev/next/concepts/endpoints-and-apis/caching.md#the-global-cache-and-redis).

Cluster messaging is best effort. Failed publishes are not retried. The `MessageScope.global` scope requires Redis and throws `StateError('Redis needs to be enabled to use this method')` without it. See [Message scope](https://docs.serverpod.dev/next/concepts/endpoints-and-apis/server-events.md#message-scope).

The [`/readyz`](https://docs.serverpod.dev/next/concepts/operations/health-checks.md) probe checks Redis only when Redis is enabled. The `/livez` probe does not.
...
To send Redis commands beyond the cache and pub/sub APIs, borrow the connection Serverpod manages. See [Send Redis commands directly](https://docs.serverpod.dev/next/concepts/endpoints-and-apis/caching.md#send-redis-commands-directly).
...
In every run mode except production, a down Redis does not stop the server. Confirm the container is running and that [`/readyz`](https://docs.serverpod.dev/next/concepts/operations/health-checks.md) reports a passing Redis check. Production does not fall back.
...
- [Configuration](https://docs.serverpod.dev/next/concepts/server-fundamentals/configuration.md): run modes, YAML files, and secrets.
- [Configuration reference](https://docs.serverpod.dev/next/concepts/lookups/configuration-reference.md): every Redis option and environment variable.
- [Caching](https://docs.serverpod.dev/next/concepts/endpoints-and-apis/caching.md): the global cache Redis backs.
- [Server events](https://docs.serverpod.dev/next/concepts/endpoints-and-apis/server-events.md): cluster messaging through Redis.
