...
https://docs.serverpod.dev/next/concepts/server-fundamentals/configuration
...
For every available option, its environment variable, config-file key, and default, see the [Configuration reference](https://docs.serverpod.dev/next/concepts/lookups/configuration-reference.md).
...
The `test` mode is used by the [test tools](https://docs.serverpod.dev/next/concepts/testing/get-started.md), which start the server against `config/test.yaml`. The other modes separate your local setup from your deployed environments. See [Running your server](https://docs.serverpod.dev/next/concepts/server-fundamentals/running-your-server.md#choose-a-run-mode) for how to pass `--mode` through `serverpod start`.

Run modes configure the server side. Your Flutter app picks the matching server address per environment on its own; see [Point the client at each environment](https://docs.serverpod.dev/next/concepts/endpoints-and-apis.md#point-the-client-at-each-environment).
...
Inside an endpoint, read a secret from the [`Session`](https://docs.serverpod.dev/next/concepts/endpoints-and-apis/sessions.md) through the `passwords` map:
...
For every `generator.yaml` option, its type and default, see the [Configuration reference](https://docs.serverpod.dev/next/concepts/lookups/configuration-reference.md#code-generation). The sections below explain the options you set most often.
...
Delete the line to stop generating test tools. See the [testing documentation](https://docs.serverpod.dev/next/concepts/testing/get-started.md) for how the test tools are used.
...
This allows you to reference module classes as `module:auth:AuthUser` in your model files. See the [modules documentation](https://docs.serverpod.dev/next/concepts/server-fundamentals/modules.md) for more information.
...
Models and the protocol file are generated in each shared package's own directory when you run `serverpod generate` from your server project. See the [shared packages documentation](https://docs.serverpod.dev/next/concepts/data-and-the-database/models/shared-packages.md) for setup, usage, and restrictions.
...
See the [serialization documentation](https://docs.serverpod.dev/next/concepts/data-and-the-database/models/custom-serialization.md) for implementing custom serializable classes.
...
Experimental features are off by default and opted into per project with the `experimental_features` key. The current one is `databaseSync`, which enables [`database: sync`](https://docs.serverpod.dev/next/concepts/data-and-the-database/database/client-side-database.md#sync-tables-with-the-server) on table models; `all` enables every available feature:
