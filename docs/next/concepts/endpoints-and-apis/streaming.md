...
https://docs.serverpod.dev/next/concepts/endpoints-and-apis/streaming
...
Each time the client calls a streaming method, a new [`Session`](https://docs.serverpod.dev/next/concepts/endpoints-and-apis/sessions.md) is created for the call, and it closes automatically when the call is over. If the WebSocket connection is lost, all streaming methods are closed on both the server and the client.
...
For more details on handling revoked authentication, refer to the section on [handling revoked authentication](https://docs.serverpod.dev/next/concepts/authentication/custom-overrides.md#handling-revoked-authentication).
...
Define the exception in a model file, like any [serializable exception](https://docs.serverpod.dev/next/concepts/endpoints-and-apis/error-handling-and-exceptions.md):
