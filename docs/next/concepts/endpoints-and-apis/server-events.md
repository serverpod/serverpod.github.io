...
Serverpod uses Redis to pass messages between servers. To send a message to another server, [enable Redis](https://docs.serverpod.dev/next/concepts/server-fundamentals/configuration.md) and then set the `global` parameter to `true` when posting a message.
