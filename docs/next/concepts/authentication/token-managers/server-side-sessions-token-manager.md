...
https://docs.serverpod.dev/next/concepts/authentication/token-managers/server-side-sessions-token-manager
...
In that case, create the token with `AuthServices.instance.tokenManager.createToken` rather than `issueToken`, which is for [sign-in flows](https://docs.serverpod.dev/next/concepts/authentication/token-managers/managing-tokens.md#issuing-tokens). Then use the returned `AuthSuccess.serverSideSessionId` to insert your metadata with the endpoint's parameters:
...
- [JWT token manager](https://docs.serverpod.dev/next/concepts/authentication/token-managers/jwt-token-manager.md): the stateless alternative.
- [Setup](https://docs.serverpod.dev/next/concepts/authentication/setup.md): where token managers are configured.
