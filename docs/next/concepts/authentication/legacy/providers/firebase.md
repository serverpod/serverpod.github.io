...
https://docs.serverpod.dev/next/concepts/authentication/legacy/providers/firebase
...
:::danger
The firebase\_service\_account\_key.json file gives admin access to your Firebase project, never store it in version control.
:::
...
Add the [serverpod\_auth\_firebase\_flutter](https://pub.dev/packages/serverpod_auth_firebase_flutter) package.
...
Where `context` is your `BuildContext`, `caller` and `authProviders` are the same as for the button. The method returns a nullable [UserInfo](https://docs.serverpod.dev/next/concepts/authentication/legacy/working-with-users.md) object, if the object is null the Sign-in failed, if not the Sign-in was successful.
