...
Quoting the values is a safeguard. YAML parses unquoted values that look like numbers as numbers instead of strings. Facebook App IDs are numeric, so an unquoted `facebookAppId` crashes startup with an "Invalid password entries" error instead of this one. On Serverpod Cloud, set them with `scloud password set` instead. See [Publishing to production](https://docs.serverpod.dev/next/concepts/authentication/providers/facebook/setup.md#publishing-to-production).
