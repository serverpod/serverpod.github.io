...
Although the anonymous identity provider can be used directly with no other configuration, it is recommended to add some form of app attestation to prevent abuse in production environments. See the [Using a token for app attestation section](https://docs.serverpod.dev/next/concepts/authentication/providers/anonymous/customizations.md#using-a-token-for-app-attestation) for more details.

For other configuration options such as callbacks (before/after account creation) and rate limiting, see the [customizations page](https://docs.serverpod.dev/next/concepts/authentication/providers/anonymous/customizations.md).

## Client-side configuration

If you have configured the `SignInWidget` as described in the [setup section](https://docs.serverpod.dev/next/concepts/authentication/setup.md#present-the-authentication-ui), the anonymous identity provider will be automatically detected and displayed in the sign-in widget as a "Continue without account" option.
