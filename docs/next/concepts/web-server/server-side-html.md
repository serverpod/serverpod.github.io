...
https://docs.serverpod.dev/next/concepts/web-server/server-side-html

Server-rendered pages generate their HTML on the server for each request, with full access to your database through the `Session`. Choose this for content pages, dashboards, and landing pages. For client-rendered apps, see [Single-page apps](https://docs.serverpod.dev/next/concepts/web-server/single-page-apps.md) instead.
...
A template can refer to a stylesheet or an image by its [cache-busted](https://docs.serverpod.dev/next/concepts/web-server/static-files.md#static-file-cache-busting) URL with the `{{{@/path}}}` tag, so the browser picks up a new version of the file as soon as it changes. Write the asset's served path after the `@`:
...
To build the versioned URL in Dart instead and pass it in as a template value, use [`assetPath()`](https://docs.serverpod.dev/next/concepts/web-server/static-files.md#generating-versioned-urls).
...
- Use [custom routes](https://docs.serverpod.dev/next/concepts/web-server/routing.md) for REST APIs and custom request handling
- Serve [static files](https://docs.serverpod.dev/next/concepts/web-server/static-files.md) for CSS, JavaScript, and images
- Add [middleware](https://docs.serverpod.dev/next/concepts/web-server/web-server-middleware.md) for cross-cutting concerns like logging and
  error handling
