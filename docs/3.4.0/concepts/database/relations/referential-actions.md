...
https://docs.serverpod.dev/3.4.0/concepts/database/relations/referential-actions
...
If the relation is defined as an [object relation](https://docs.serverpod.dev/3.4.0/concepts/database/relations/one-to-one.md#with-an-object), the default behavior is `NoAction` for both onUpdate and onDelete.

```yaml
parent: Model?, relation(onUpdate=NoAction, onDelete=NoAction)
```

If the relation is defined as an [id relation](https://docs.serverpod.dev/3.4.0/concepts/database/relations/one-to-one.md#with-an-id-field), the default behavior is `NoAction` for onUpdate and `Cascade` for onDelete.
