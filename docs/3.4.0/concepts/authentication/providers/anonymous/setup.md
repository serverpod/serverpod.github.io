...
https://docs.serverpod.dev/3.4.0/concepts/authentication/providers/anonymous/setup
...
Although the Anonymous IDP can be used directly with no other configuration, it is recommended to add some form of app attestation to prevent abuse on production environments. See the [Using a token for app attestation section](https://docs.serverpod.dev/3.4.0/concepts/authentication/providers/anonymous/configuration.md#using-a-token-for-app-attestation) for more details.

For other configuration options such as callbacks (before/after account creation) and rate limiting, see the [configuration section](https://docs.serverpod.dev/3.4.0/concepts/authentication/providers/anonymous/configuration.md).

## Client-side configuration

If you have configured the `SignInWidget` as described in the [setup section](https://docs.serverpod.dev/3.4.0/concepts/authentication/setup.md#present-the-authentication-ui), the Anonymous identity provider will be automatically detected and displayed in the sign-in widget as a "Continue without account" option.
