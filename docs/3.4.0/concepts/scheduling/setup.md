...
https://docs.serverpod.dev/3.4.0/concepts/scheduling/setup

Serverpod supports scheduling future work with the `future call` feature. Future calls are calls that will be invoked at a later time. An example is if you want to send a drip-email campaign after a user signs up. You can schedule a future call for a day, a week, a month, or a [recurring interval](https://docs.serverpod.dev/3.4.0/concepts/scheduling/recurring-task.md). The calls are stored in the database, so they will persist even if the server is restarted.
