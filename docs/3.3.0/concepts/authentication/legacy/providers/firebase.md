...
https://docs.serverpod.dev/3.3.0/concepts/authentication/legacy/providers/firebase
...
Where `context` is your `BuildContext`, `caller` and `authProviders` are the same as for the button. The method returns a nullable [UserInfo](https://docs.serverpod.dev/3.3.0/concepts/authentication/legacy/working-with-users.md) object, if the object is null the Sign-in failed, if not the Sign-in was successful.
