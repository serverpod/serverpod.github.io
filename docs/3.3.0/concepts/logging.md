...
https://docs.serverpod.dev/3.3.0/concepts/logging
...
- `SERVERPOD_SESSION_CONSOLE_LOG_FORMAT`: The format for console logging (`text` or `json`). See [configuration](https://docs.serverpod.dev/3.3.0/concepts/configuration.md).
...
Duration strings for the cleanup interval and retention period use the same format as in [models](https://docs.serverpod.dev/3.3.0/concepts/models.md#duration): e.g. `30d`, `6h`, `1d 2h 30min`.
...
:::info
You can use the companion app  **[Serverpod Insights](https://docs.serverpod.dev/3.3.0/tools/insights.md)** to read, search, and configure the logs.
:::
