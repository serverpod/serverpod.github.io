...
When the `table` keyword is added to the model, the `serverpod generate` command will generate new methods for [interacting](https://docs.serverpod.dev/concepts/database/crud.md) with the database. The addition of the keyword will also be detected by the `serverpod create-migration` command that will generate the necessary [migrations](https://docs.serverpod.dev/concepts/database/migrations.md) needed to update the database.
...
For a complete guide on how to work with relations see the [relation section](https://docs.serverpod.dev/concepts/database/relations/one-to-one.md).
