...
:::warning
When using `ignoreConflicts` with models that have [non-persistent fields](https://docs.serverpod.dev/concepts/database/models.md#non-persistent-fields), each row is inserted individually instead of in a single batch. This is necessary because the database cannot report which rows were skipped in a batch insert, making it impossible to correctly match non-persistent field values back to inserted rows. For large numbers of rows, this can cause performance issues. Consider removing non-persistent fields from the model or inserting in smaller batches.
:::
...
This operation returns the first model matching the filtering criteria or `null`. See [filter](https://docs.serverpod.dev/concepts/database/filter.md) and [sort](https://docs.serverpod.dev/concepts/database/sort.md) for all filter operations.
...
See [filter](https://docs.serverpod.dev/concepts/database/filter.md) and [sort](https://docs.serverpod.dev/concepts/database/sort.md) for all filter and sorting operations and [pagination](https://docs.serverpod.dev/concepts/database/pagination.md) for how to paginate the result.
...
The `updateWhere` method updates all rows matching the where expression, modifying only the specified columns. The method returns a list of the updated rows. If no rows match the criteria, an empty list is returned. See [filter](https://docs.serverpod.dev/concepts/database/filter.md) for all available filtering operations.

The method also supports [pagination](https://docs.serverpod.dev/concepts/database/pagination.md) and [ordering](https://docs.serverpod.dev/concepts/database/sort.md):
...
You can also do a [filtered](https://docs.serverpod.dev/concepts/database/filter.md) delete and delete all entries matching a `where` query, by using the `deleteWhere` method.
...
Count is a special type of query that helps counting the number of rows in the database that matches a specific [filter](https://docs.serverpod.dev/concepts/database/filter.md).
