...
https://docs.serverpod.dev/concepts/database/filter
...
:::tip
For optimal performance with vector similarity searches, consider creating specialized vector indexes (HNSW or IVFFLAT) on your vector fields. See the [Vector indexes](https://docs.serverpod.dev/concepts/database/indexing.md#vector-indexes) section for more details. To fine-tune query execution, you can also set appropriate [runtime parameters](https://docs.serverpod.dev/concepts/database/runtime-parameters.md) for vector queries.
:::

## Relation operations

If a relation between two models is defined a [one-to-one](https://docs.serverpod.dev/concepts/database/relations/one-to-one.md) or [one-to-many](https://docs.serverpod.dev/concepts/database/relations/one-to-many.md) object relation, then relation operations are supported in Serverpod.
