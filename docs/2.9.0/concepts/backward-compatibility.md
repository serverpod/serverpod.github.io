...
https://docs.serverpod.dev/2.9.0/concepts/backward-compatibility
...
3. **Avoid changing or removing fields and types in the serialized classes.** However, you are free to add new fields as long as they are nullable.

## Managing breaking changes with endpoint inheritance

An [endpoint sub-class](https://docs.serverpod.dev/2.9.0/concepts/working-with-endpoints.md) can be useful when you have to make a breaking change to an entire endpoint but need to keep supporting existing clients. Doing so allows you to share most of its implementation with the old endpoint.
